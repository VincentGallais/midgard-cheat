...
export class CheatDetection {
  ...
  async handleError(reply, error) {
    console.log(error)
    this.logger.error(`Error while scanning user leads`, error)

    return reply.status(500).send({
      error: 'Internal Server Error',
      message: `An error occurred while scanning user leads.`
    })
  }
  ...
  registerEndpoints(fastifyInstance) {
    fastifyInstance.post('/api/cheat-detection/scan-lead', async (request, reply) => {
      try {
        ...
        return this.handleError(reply, error, 'Scan Leads')
      }
    })
    ...
    fastifyInstance.post('/api/cheat-detection/scan-bidding', async (request, reply) => {
      try {
        ...
        return this.handleError(reply, error, 'Scan Bidding')
      }
    })
  }
}
